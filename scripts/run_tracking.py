#!/usr/bin/env python3
"""Run Stage 6 YOLO26n with persistent Ultralytics ByteTrack state."""
...
from vision_analytics.tracking.schema import TRACK_FIELDS, TrackRecord
...
STAGE4_BENCHMARK = PROJECT_ROOT / "outputs" / "analytics" / "stage4_detection_benchmark.csv"
TRACK_OUTPUT_DIR = PROJECT_ROOT / "outputs" / "tracks" / "stage6"
BENCHMARK_PATH = PROJECT_ROOT / "outputs" / "analytics" / "stage6_tracking_benchmark.csv"

STAGE6_BENCHMARK_FIELDS = (
    "video_id",
    "source_id",
    "model",
    "device",
    ...
    "elapsed_seconds",
    "processing_fps",
    "stage4_processing_fps",
    "fps_ratio_vs_stage4",
    "track_observations",
    "unique_track_ids_observed",
    "output_path",
    "status",
    "validation_message",
)
...
def load_stage4_fps() -> dict[str, float]:
    if not STAGE4_BENCHMARK.is_file():
        return {}
    with STAGE4_BENCHMARK.open(newline="", encoding="utf-8") as handle:
        return {
...
def write_tracking_outputs(
    ...
    *,
    ...
    with csv_path.open("w", newline="", encoding="utf-8") as handle:
        writer = csv.DictWriter(handle, fieldnames=TRACK_FIELDS, lineterminator="\n")
        writer.writeheader()
        writer.writerows(record.to_row() for record in records)

    ids_by_class: dict[str, set[int]] = defaultdict(set)
    for record in records:
        ids_by_class[record.class_name].add(record.track_id)
    observed_ids = {record.track_id for record in records}
    summary = {
        ...
        "track_observations": len(records),
        "unique_track_ids_observed": len(observed_ids),
        "class_track_ids_observed": {
            class_name: len(track_ids)
            for class_name, track_ids in sorted(ids_by_class.items())
        },
        "diagnostic_warning": (
            "Track IDs are video-scoped diagnostics, not unique traffic or business counts."
        ),
...
def main() -> int:
    ...
    if not torch.backends.mps.is_built() or not torch.backends.mps.is_available():
        print("Stage 6 requires Apple MPS; CPU fallback is disabled", file=sys.stderr)
        return 1
    if not MODEL_PATH.is_file():
        print(f"Model weights not found: {MODEL_PATH}", file=sys.stderr)
        return 1

    stage4_fps = load_stage4_fps()
    benchmark_rows: list[dict[str, object]] = []
    for source in load_runtime_sources():
        input_relative = Path(source["local_path"])
        video_id = input_relative.stem
        output_relative = Path("outputs/videos/stage6") / f"{video_id}_stage6.mp4"
        tracks_csv = TRACK_OUTPUT_DIR / f"{video_id}_tracks.csv"
        summary_json = TRACK_OUTPUT_DIR / f"{video_id}_summary.json"
        records: list[TrackRecord] = []

        # One wrapper scopes ByteTrack state and IDs to this video. Persist=True
        # keeps that state alive for every sequential frame processed below.
        ...
        print(f"Tracking {source['source_id']} ({input_relative.name})...", flush=True)
        benchmark = process_video(
            PROJECT_ROOT / input_relative,
            PROJECT_ROOT / output_relative,
            video_id=video_id,
            source_id=source["source_id"],
            frame_processor=track_and_draw,
        )
        ...
        observed_ids = {record.track_id for record in records}
        prior_fps = stage4_fps.get(source["source_id"], 0.0)
        ...
        benchmark_rows.append(
            {
                ...
                "stage4_processing_fps": prior_fps,
                "fps_ratio_vs_stage4": (
        ...
        print(
            ...
            f"{benchmark['processing_fps']} FPS, {len(records)} track observations, "
            f"{len(observed_ids)} unique track IDs observed (diagnostic only)",
            flush=True,
        )
    ...
    with BENCHMARK_PATH.open("w", newline="", encoding="utf-8") as handle:
        writer = csv.DictWriter(
            handle, fieldnames=STAGE6_BENCHMARK_FIELDS, lineterminator="\n"
        )
        writer.writeheader()
